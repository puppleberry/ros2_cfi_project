...
#include <fstream>
#include <execinfo.h>
#include <link.h>  // dl_iterate_phdr를 위해 추가
...
void cfi_init() {
    ...
    LOG("=== ROS2 CFI Day 6 (Improved Version) ===");
    LOG("PID: %d", getpid());
...
void cfi_fini() {
    ...
    if (getenv("ROS2_CFI_SAVE_STATS")) {
        std::ofstream stats_file("/tmp/day6_cfi_stats.txt");
        stats_file << "Day 6 CFI Statistics\n";
        stats_file << "====================\n";
        ...
        stats_file << "Hook status: " << (g_hook_successful.load() ? "SUCCESS" : "FAILED") << "\n";
        stats_file.close();
        LOG("Statistics saved to /tmp/day6_cfi_stats.txt");
    }
}
